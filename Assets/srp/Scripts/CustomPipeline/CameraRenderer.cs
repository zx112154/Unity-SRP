...
public partial class CameraRenderer
{
    ...
    /// 当前相机Buffer
    /// </summary>
    private CommandBuffer m_cameraBuffer = new CommandBuffer { name = "Render Camera" };
    ...
    /// 最大灯光数量
    /// </summary>
    private const int m_maxVisibleLights = 16;
    ...
    private void setUp()
    {
        ...
        m_cameraBuffer.ClearRenderTarget((m_camera.clearFlags & CameraClearFlags.Depth) != 0, (m_camera.clearFlags & CameraClearFlags.Color) != 0, m_rpp.clearColor);
    ...
    private void executeBuffer(CommandBuffer _cmd)
    {
    ...
    private bool cull()
    {
        ...
        {
            p.shadowDistance = Mathf.Min(m_rpp.shadowDistance, m_camera.farClipPlane);
    ...
    {
        float tileSize = m_rpp.shadowMapSize / 2;

        m_cascadedShadowMap = setShadowRenderTarget();
        m_shadowBuffer.BeginSample("Render Cascaded Shadows");
        m_shadowBuffer.SetGlobalVector(m_globalShadowDataId, new Vector4(0.0f, m_rpp.shadowDistance * m_rpp.shadowDistance));
        executeBuffer(m_shadowBuffer);
        ...
        //构建级联世界到阴影贴图空间矩阵
        for (int i = 0; i < m_rpp.shadowCascades; i++)
        {
            Matrix4x4 viewMatrix, projectionMatrix;
            ShadowSplitData splitData;
            m_cullingResults.ComputeDirectionalShadowMatricesAndCullingPrimitives(0, i, m_rpp.shadowCascades, m_rpp.shadowCascadeSplit,
        ...
        m_shadowBuffer.SetGlobalVector(m_cascadedShadowMapSizeId, new Vector4(invShadowMapSize, invShadowMapSize, m_rpp.shadowMapSize, m_rpp.shadowMapSize));
    ...
    {
        RenderTexture texture = RenderTexture.GetTemporary(m_rpp.shadowMapSize, m_rpp.shadowMapSize, 16, RenderTextureFormat.Shadowmap);
