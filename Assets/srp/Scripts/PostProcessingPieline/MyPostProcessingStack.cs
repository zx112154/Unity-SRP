...
public class MyPostProcessingStack : ScriptableObject
{
    ...
    public void RenderAfterTransparent()
    {
        if (m_blurStrength > 0)
        {
            if (m_toneMapping || m_param.samples > 1)
            {
                ...
                blur(s_resolvedTexId);
                m_param.cb.ReleaseTemporaryRT(s_resolvedTexId);
            }
            else
            {
                blur(m_param.cameraColorId);
            }
        }
    ...
    private void blur(int dstTex)
    {
        ...
        {
            blit(dstTex, s_tempTexId, EPass.Blur);
        ...
        {
            blit(dstTex, s_tempTexId, EPass.Blur);
            blit(dstTex, BuiltinRenderTextureType.CameraTarget, EPass.Blur);
        }
